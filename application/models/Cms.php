...
Class Cms extends CI_Model{
	...
	public function cek_login_user(){
		...
		if(isset($_SESSION['current_token'])){
			$cek = query("
				SELECT username, name, email, priviledge FROM cms_user WHERE username IN
				(
					SELECT username FROM cms_user_log WHERE token = ".quote($_SESSION['current_token'])." AND expired > ".quote(date("Y-m-d H:i:s"))."
				)
				");
			if($cek->num_rows() > 0){
				$row = $cek->row_array();
