...
Class Login extends CI_Model{
	...
	public function login_process_user($username, $password){
		$sql = query("SELECT * FROM cms_user WHERE username = ".quote($username));
		if($sql->num_rows() > 0){
			$row = $sql->row_array();
			if(password_verify($password, $row['password'])){
				return true;
	...
	public function validate_session_user(){
		$cek = query("SELECT * FROM cms_user_fail WHERE stat = 0 AND ip = ".quote($_SERVER['REMOTE_ADDR']));
		$max_try = get_setting("max_login_try");
		if($cek->num_rows() >= $max_try)
			return false;
		else
	...
	public function login_record_user($username){
		$s = $_SERVER;
		$tgl = date("Y-m-d H:i:s");
		$expired = date("Y-m-d H:i:s",strtotime($tgl) + 12 * 60 * 60);
		...
		//save data to database
		if(query("INSERT INTO cms_user_log VALUES (NULL, ".quote($username).", ".quote($tgl).", ".quote($expired).", ".quote($token).", ".quote($s['REMOTE_ADDR']).", ".quote($s['HTTP_USER_AGENT']).")")){
	...
	public function fail_record_user($username){
		$s = $_SERVER;
		$tgl = date("Y-m-d H:i:s");

		query("INSERT INTO cms_user_fail VALUES (NULL, ".quote($username).", ".quote($tgl).", ".quote($s['REMOTE_ADDR']).", ".quote($s['HTTP_USER_AGENT']).", 0)");
